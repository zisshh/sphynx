...
func main() {
	...
	r.Use(handlers.MiddlewareSlidingWindowRateLimiting)       // Add rate limiting middleware
	...
	r.HandleFunc("/access/vs/rate-limits", handlers.UpdateRateLimit).Methods("POST")
	r.HandleFunc("/access/vs/{vs_id:[0-9]+}/rules", handlers.AddContentRoutingRule).Methods("POST")
	r.HandleFunc("/access/vs/{vs_id:[0-9]+}/rules", handlers.GetContentRoutingRules).Methods("GET")
	r.HandleFunc("/access/vs/{vs_id:[0-9]+}/rules/{rule_index:[0-9]+}", handlers.DeleteContentRoutingRule).Methods("DELETE")

	// Serve the admin dashboard (must be registered last; it is a catch-all)
	r.PathPrefix("/").Handler(http.FileServer(http.Dir("./frontend")))

	// Start the HTTP server
	log.Printf("Server running on port %d  (dashboard: http://localhost:%d/)", 8080, 8080)
