...
// GetContentBasedServer selects a server based on content routing rules
func GetContentBasedServer(vs *models.VirtualService, req *http.Request) (*models.Server, error) {
	for _, rule := range vs.ContentRoutingRules {
		if rule.Matches(req) {
			for _, server := range vs.ServerList {
				if server.Name == rule.ServerName && server.Health {
					vs.Logger.Infof("Content-based routing matched rule for server: %s", server.Name)
					return server, nil
				}
			}
		}
	}
	return nil, errors.New("no matching content-based routing rule or healthy servers")
}
...
func GetHealthyServer(vs *models.VirtualService, req *http.Request) (*models.Server, error) {
	switch vs.Algorithm {
	case "round_robin":
	...
	case "content_based":
		return GetContentBasedServer(vs, req)
	default:
